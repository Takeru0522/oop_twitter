...
class Task extends Model
{
    ...
    public function create($data)
    {
        ...
        $stmt = $this->db_manager->dbh->prepare('INSERT INTO ' . $this->table . ' (contents, created, tweetedBy) VALUES (?, ?, ?)');
        $stmt->execute($data);
    }
    // Search Function
    ...
    // Twitter なので、タイトルは無くしました。
    // よってfindByTitleもfindByContentsに変更。
    ...
    public function getUserTweets($data)
    {
        $stmt = $this->db_manager->dbh->prepare('SELECT * FROM ' . $this->table . ' WHERE tweetedBy = ? ');
        $stmt->execute($data);
        $tasks = $stmt->fetchAll();
